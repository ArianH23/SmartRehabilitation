...
for i in range(1, n+1):
    l_names.extend([f"r{i}x", f"r{i}y"])
  ...
  for index, row in mp_yolo_df.iterrows():
    ...
    for ind, hand_landmarks in enumerate(results.multi_hand_landmarks):
      ...
      for mark in hand_landmarks.landmark:
        ...
        if label == 'Left'and len(landmarks_left)<42:
          landmarks_left.extend([dist_x, dist_y])

        if label == 'Right' and len(landmarks_right)<42:
          landmarks_right.extend([dist_x, dist_y])

    if landmarks_left == []: landmarks_left = [0]*42
    if landmarks_right == []: landmarks_right = [0]*42

    aux_row.extend(landmarks_left)
...
df.to_csv('dataset_dist_to_min/MPandYOLOandLM_dist2Hands.csv', index=False)
