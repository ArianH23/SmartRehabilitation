...
df = pd.DataFrame(columns=['col{}'.format(i) for i in range(1, 66)])
i = 0
with mp_hands.Hands(
  ...
  for path, subdirs, files in os.walk('frames'):
    ...
    #   print('Handedness:', results.multi_handedness)
      ...
      x_dist = [int(round(image_width * x)) - min_int_x for x in hc_x]
      y_dist = [int(round(image_height * y)) - min_int_y for y in hc_y]
      z_dist = [z - min_z for z in hc_z]
    ...
    #   print(z_dist)
      ...
      dists_list = x_dist + y_dist + z_dist
...
df.to_csv('dataset_dist_to_min/data.csv', index=False)
