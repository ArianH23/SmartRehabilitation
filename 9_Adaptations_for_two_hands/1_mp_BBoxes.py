...
with mp_hands.Hands(
  ...
  for path, subdirs, files in os.walk('EKDataset'):
    for name in files:
      ...
      for ind, hand_landmarks in enumerate(results.multi_hand_landmarks):
        ...
        if label not in image_path: continue
      ...
      # cv2.imwrite(
      #     'EKBBoxes/'+ name[:-4] + '.png', cv2.flip(annotated_image, 1))
      # cv2.imwrite(
      #     'EKBBoxes/'+ name[:-4] + 'rev.png', annotated_image)
...
df.to_csv('EKdata.csv', index=False)
