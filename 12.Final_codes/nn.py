...
# X_train = X[:separator]
# y_train = y_df[:separator]
# X_val = X[separator:]
# y_val = y_df[separator:]

print(y_train)
print(y_train[0].value_counts())
print(y_train[1].value_counts())
print(y_train[2].value_counts())
print(X_train)
if normalize:
    max_dist_train = X_train['depth_dist'].abs().max()
    max_dist_val = X_val['depth_dist'].abs().max()

    X_train['picture_name'] = y_train['picture_name']
    X_val['picture_name'] = y_val['picture_name']
    
    X_train = X_train.apply(apply_func_adapted, axis=1, args=([max_dist_train]))
    X_val = X_val.apply(apply_func_adapted, axis=1, args=([max_dist_train]))

    X_train.drop('picture_name', axis=1, inplace=True) 
    X_val.drop('picture_name', axis=1, inplace=True) 
    
    X_train = torch.tensor(X_train.values, dtype=torch.float32)
    X_val = torch.tensor(X_val.values, dtype=torch.float32)
print(X_train)

y_val_names = y_val[['picture_name', 'handedness']]
y_train.drop('picture_name', axis=1, inplace=True)
y_train.drop('handedness', axis=1, inplace=True)
y_val.drop('picture_name', axis=1, inplace=True)
y_val.drop('handedness', axis=1, inplace=True)
...
learning_rate = 0.01
class_weights = torch.tensor([1-0.3618, 1-0.35014, 1-0.28802],dtype=torch.float)
# class_weights = torch.nn.Parameter( torch.FloatTensor(0.638168428, 0.649855908, 0.711975664, device="cuda") )
...
datasettrain = torch.utils.data.TensorDataset(X_train, y_train)
...
for epoch in range(n_epochs):
    ...
    model.train()
    with tqdm.trange(len(dataloader), unit="batch", mininterval=0) as bar:
    ...
    writer.add_scalar('Loss/test', ce, epoch)
...
date = datetime.datetime.now().strftime("%H:%M:%S-%B-%d")
model_name = date + '_' + dataset_name + '_acc' + str(best_acc)[:5] + '.pth'

torch.save({'model_state_dict': model.state_dict()}, 'models/' + model_name)
...
sns.heatmap(df_cm_perc, annot=True, cmap='Blues')
plt.savefig('output_perc.png')
plt.figure(figsize = (12,7))
sns.heatmap(df_cm_int, annot=True, cmap='Blues',fmt="d")
plt.savefig('output_int.png')

# XAI: if it works, it might take a long time to finish
