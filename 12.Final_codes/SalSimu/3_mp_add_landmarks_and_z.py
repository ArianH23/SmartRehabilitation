...
mp_yolo_df = pd.read_csv('SaladRelabel_mp_yolo.csv')
...
with mp_hands.Hands(
  ...
  for index, row in mp_yolo_df.iterrows():
    ...
    if not os.path.exists(image):
            image = 'frames_salad/'+'none'+'/'+name
            if not os.path.exists(image):
                image = 'frames_salad/'+'precision'+'/'+name
                if not os.path.exists(image):
                    image = 'frames_salad/'+'power'+'/'+name
  ...
  df.to_csv('Simulation_mp_yolo_mp.csv', index=False)
