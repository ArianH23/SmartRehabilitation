...
with mp_hands.Hands(
  ...
  for path, subdirs, files in os.walk('frames_simulation/'):
    for name in files:
      ...
      for fold in possible_folders:
        ...
        image_path = os.path.join(ori_path + '/' + name)
        image = cv2.flip(cv2.imread(image_path), 1)
      ...
      for ind, hand_landmarks in enumerate(results.multi_hand_landmarks):
        ...
        elif 'none' in path: d_list.append('none')
        else: d_list.append('none')
...
df.grasp.value_counts()
df.to_csv('Simulation_mp.csv', index=False)
