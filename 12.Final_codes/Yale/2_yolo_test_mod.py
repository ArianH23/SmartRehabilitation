...
for index, row in mp_df.iterrows():
    ...
    for fold in possible_folders:
        image = file_name#[16:]# "EpicKitchen/" + fold +'/'+ file_name
        # print(image,'XD')
        results = model.predict(image, save_txt=True, save=True, classes=[13,65,74,79,39,43,70,45,59,73,44,64,75,32,35,24,41,67,56,60,2,61,71],device=1,conf=0.15)#[39, 41, 42, 43, 46, 47, 64, 65, 67, 76]) 
        ...
        if reader is not None:
            ...
            for index2, objectRow in reader.iterrows():
                ...
                width = int(np.round(objectRow[3]*640))
                min_x = max(0, int(np.round(objectRow[1]*640)) - (width // 2))

                max_x = int(np.round(objectRow[1]*640)) + width // 2
                width = max_x - min_x
                ...
                if 'AFP1920' in fold:
                    ...
                    height = int(np.round(objectRow[4]*640))
                    # print(objectRow[0],objectRow[1],objectRow[2],objectRow[3],objectRow[4],np.round(objectRow[4]*1920), height)
                    min_y = max(0, int(np.round(objectRow[2]*640)-420) - height // 2)
                    max_y = int(np.round(objectRow[2]*640)-420) + height // 2
                    height = max_x - min_x
                    ...
                    height = int(np.round(objectRow[4]*480))
                    min_y = max(0, int(np.round(objectRow[2]*480)) - height // 2)

                    max_y = int(np.round(objectRow[2]*480)) + height// 2
                    height = max_y - min_y
...
bb_df.to_csv('yale_mp_yolo.csv', index = False)