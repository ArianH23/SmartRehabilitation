...
with mp_hands.Hands(
  ...
  for path, subdirs, files in os.walk('frames_simulation_partial'):
    for name in files:
      ...
      cv2.imwrite(
          'mediapipe_photos_simulation/'+ name[:-4] + '.png', cv2.flip(annotated_image, 1))
      cv2.imwrite(
          'mediapipe_photos_simulation/'+ name[:-4] + 'rev.png', annotated_image)
...
df.to_csv('dataset_dist_to_min/mp_bb_simu_part.csv', index=False)
